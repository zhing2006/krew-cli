...
- 新增 `/dream` slash 命令，支持 `global|agent|all` scope 和 `@agent` 寻址（不支持 `@all`，每次指定单个 Agent）
- 构建 consolidation prompt（3 阶段：Orient → Consolidate → Prune），根据 scope 动态注入目标目录
- `/dream` 以 **whisper 消息**方式注入当前会话，确保 dream 过程（含私有记忆读取）对其他 Agent 不可见
- Dream 执行期间通过 `exclude_tools` 收窄工具集，仅保留 `read_file`/`write_file`/`edit_file`/`glob`/`grep`，排除 `shell`/`fetch_url`/`activate_skill`/`run_agent`
