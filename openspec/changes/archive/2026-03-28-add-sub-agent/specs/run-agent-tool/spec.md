...
2. 从 `ToolContext.tool_registry` 中通过 downcast 获取父 Agent 的 `Arc<ToolRegistry>`（含 MCP tools）
3. 直接使用构造时存储的父 Agent 运行时资源：`Arc<dyn LlmClient>`（同 client）、`ApprovalCache`（同 cache）、`SamplingConfig`（同 sampling）、`ApprovalMode`（同 approval mode）
4. 使用 `SubAgentDef` 的 `system_prompt` 替换 system prompt
5. 构建独立的 messages 列表：`[system_prompt, user(task)]`
6. 从 `ToolRegistry` 构建 `tool_defs` 时过滤掉 `run_agent`，启动 agent loop
7. 阻塞消费 `AgentEvent` 流直到收到 `Done` 或 `Error`
8. 返回 `final_text` 作为 tool result
...
### Requirement: run_agent Approval 转发与 ToolRegistry 传入
`krew-tools::ToolContext` SHALL 新增以下字段（默认 `None`）：
- `parent_event_tx: Option<Box<dyn Any + Send + Sync>>` — 用于 Approval 转发
- `tool_registry: Option<Box<dyn Any + Send + Sync>>` — 用于 Sub-Agent tool dispatch

`krew-core` 的 `create_tool_context()` 在处理 `run_agent` tool 时 SHALL 将父 Agent 当前 turn 的 `UnboundedSender<AgentEvent>` 和 `Arc<ToolRegistry>` 分别装箱后设入这两个字段。

`RunAgentTool::execute()` SHALL 通过 `downcast_ref` 取回具体类型。`tool_registry` 取回失败时 SHALL 返回 `ToolError::Execution`。Sub-Agent 创建独立的 `(tx, rx)` 对，`RunAgentTool::execute()` 消费 Sub-Agent 的 `rx` 时，遇到 `ApprovalRequest` 事件 SHALL 通过取回的 parent sender 转发给 TUI。

`RunAgentTool` 自身 SHALL NOT 持有对 `ToolRegistry` 的任何引用（无 `Weak` 也无 `Arc`），以避免注册时 `Arc::get_mut` 因引用计数而失败。
