...
### Requirement: 消息格式转换
客户端 SHALL 将统一的 `ChatMessage` 列表转换为 OpenAI Chat Completions 的 `messages` 数组格式。

#### Scenario: 基本角色映射
- **WHEN** `ChatMessage` 的 role 为 System/User/Assistant/Tool
- **THEN** SHALL 映射为 OpenAI 的 `"system"`/`"user"`/`"assistant"`/`"tool"` role

#### Scenario: 其他 Agent 回复的 role 处理
- **WHEN** `ChatMessage` 的 role 为 Assistant 且 `agent_name` 不等于当前 agent
- **THEN** SHALL 根据 `other_agent_role` 参数决定使用 `"user"` 或 `"assistant"` role
