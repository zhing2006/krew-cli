...
- **display_name** — 显示名称（如 `GPT-5.6`、`Claude Opus`）
...
- **enable_thinking** — 是否请求显示思考/推理摘要
- **thinking_effort** — 思考力度（none / low / medium / high / xhigh / max）
- **reasoning_mode** — GPT-5.6 推理模式（standard / pro）
- **reasoning_context** — GPT-5.6 推理上下文（auto / current_turn / all_turns）
...
[gpt] GPT-5.6:                       ← Agent 回复：[name] 带颜色 + 显示名
  Agent 的回复内容，缩进显示
...
通过 `[[allow_rules]]`、`[[deny_rules]]`、`[[ask_rules]]` 进行细粒度权限控制。保护路径（`.git/`、`.env` 等）始终受保护，即使 `full-auto` 模式也不例外。
...
| `denied` | 工具调用被规则拒绝或非交互模式自动拒绝 |
| `text` | Agent 文本回复 |

#### 4.7.5 工具审批

`-p` 模式下使用 `full-auto` 审批策略，但 **deny 规则**、**ask 规则**和**保护路径免疫**仍然生效。需要用户确认的工具调用（ask 规则触发或默认审批）会被**自动拒绝**（fail-closed），因为非交互模式无法向用户确认。可通过 `[[allow_rules]]` 预授权特定工具以在 prompt 模式下使用。
...
provider = "openai"
model = "gpt-5.6"
api_type = "responses"           # 官方 OpenAI 默认 responses；兼容服务默认 chat
color = "green"
system_prompt = ""
tools = true
enable_web_search = false
enable_thinking = true           # 请求显示推理摘要
thinking_effort = "medium"      # none | low | medium | high | xhigh | max
# reasoning_mode = "pro"         # standard | pro
# reasoning_context = "all_turns" # auto | current_turn | all_turns
...
Agent 可跨会话持久化记忆，在新会话中保留对用户、项目和自身行为偏好的认知。

#### 4.11.1 两层存储

| 层级 | 路径 | 范围 | 记忆类型 |
| ---- | ---- | ---- | -------- |
| Global | `.krew/memory/` | 所有 Agent 共享 | `user`（用户角色偏好）、`project`（项目上下文）、`reference`（外部资源指针） |
| Per-Agent | `.krew/memory/agents/{name}/` | 单个 Agent 私有 | `feedback`（行为反馈） |

归属规则：用户身份、项目事实、外部链接对所有 Agent 一样（Global），但行为反馈（"别用 emoji"、"多写注释"）是针对特定 Agent 的（Per-Agent）。

#### 4.11.2 读写机制

- Agent 通过已有的 `read_file` / `write_file` 工具主动读写记忆文件
- 每次 Agent turn 时，MEMORY.md 索引内容自动注入 system prompt
- `tools = false` 的 Agent 仅收到索引内容（只读），不注入写入指令
- 零额外 API 成本——不做后台自动提取、不做 AI 相关性筛选

#### 4.11.3 文件格式

- 记忆文件为纯 Markdown，无 frontmatter
- `MEMORY.md` 为索引文件，每行一条 `- [Title](file.md) — hook`
- MEMORY.md 最多 200 行 / 25KB，超出时截断并附带警告

#### 4.11.4 审批

`.krew/memory/**` 路径自动豁免 DANGEROUS_DIRECTORIES 审批——Agent 读写记忆无需用户确认。用户仍可通过 `deny_rules` 或 `ask_rules` 覆盖。
...
┌──────────────────────────────────────────────────────────────────────────┐
│ Krew CLI v0.6.0                                                        │
│ Agents: [gpt] GPT-5.6 | [opus] Claude Opus | [gemini] Gemini 3.1 Pro  │
...
- ESC 取消当前 Agent 流式输出
- 双击 Ctrl+C 退出程序
- **待发送消息（Pending Message）**：Agent 响应期间，Enter 将消息排队（需含 `@`/`#` 寻址），队列满时 Enter 回退为换行。待发送消息显示在 viewport 顶部（`⏳` 前缀 + 路由色点），调度完成后自动提交。按 `↑` 可撤销最后一条 pending 到输入框
...
**用户配置初始化**：循环式 Provider 创建向导（选择类型 → 输入名称 → 选择 API Key 方式 → 可选参数）。如果 `base_url` 以 `/v1` 结尾，自动去掉以避免路径重复。
...
OpenAI 兼容 Provider 在 Agent 创建时默认选择 Chat Completions，也可改选 Responses；官方 OpenAI 自动使用 Responses。官方 GPT-5.6 模型额外提供六档 reasoning effort 和 reasoning summary 开关，Pro/context 仍由高级用户手工配置。
