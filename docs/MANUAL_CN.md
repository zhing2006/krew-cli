...
12. [会话管理](#12-会话管理)
13. [Prompt 模式](#13-prompt-模式)
14. [项目指令 (AGENTS.md)](#14-项目指令-agentsmd)
15. [文件路径与加载优先级](#15-文件路径与加载优先级)
16. [快捷键](#16-快捷键)
17. [常见问题](#17-常见问题)
...
- 添加更多 Agent → 看[指南 1：多 Provider 配置](#指南-1多-provider-配置openai--anthropic)
- 试试密语模式 → 输入 `#opus 悄悄话`
...
把 Provider 和 API Key 放在用户级配置中，所有项目共享。
...
Options:
  ...
  -p, --prompt <PROMPT>         非交互式 prompt 模式（见 §12）
...
检查以下几点：
1. `.md` 文件是否在[发现路径](#15-文件路径与加载优先级)中（如 `.krew/commands/my-command.md`）
