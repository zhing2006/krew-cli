...
13. [Sub-Agent 系统（实验性）](#13-sub-agent-系统实验性)
14. [Agent Memory](#14-agent-memory)
15. [会话管理](#15-会话管理)
16. [Prompt 模式](#16-prompt-模式)
17. [项目指令 (AGENTS.md)](#17-项目指令-agentsmd)
18. [文件路径与加载优先级](#18-文件路径与加载优先级)
19. [快捷键](#19-快捷键)
20. [常见问题](#20-常见问题)
...
不想每次都确认 `ls` 和 `cargo build`？添加权限规则：
...
Shell 模式使用 `*` 通配符——`cargo *` 匹配 `cargo build --release`、`cargo test` 等。
...
Options:
  ...
  -p, --prompt <PROMPT>         非交互式 prompt 模式（见 §16）
...
权限规则是**顶层**数组（不在 `[settings]` 下）。按 deny → ask → allow 顺序评估。
...
enable_thinking = false          # 请求显示推理摘要（默认 false）
# thinking_effort = "medium"     # 推理力度: none | low | medium | high | xhigh | max
                                 # GPT-5.6 支持全部六档；不支持的模型/Provider 可能将
                                 # xhigh/max 降级为 high。

# OpenAI 专用
# api_type = "responses"         # 官方 OpenAI 默认 Responses；自定义 URL 默认 Chat
...
**OpenAI GPT-5.6 说明**（`gpt-5.6`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`）：

- `gpt-5.6` 是别名并路由到 Sol；fallback 模型列表也包含 Sol、Terra、Luna 的显式模型 ID。
- 支持六档推理力度：`none`、`low`、`medium`、`high`、`xhigh`、`max`。显式 `thinking_effort` 会独立控制推理计算量，即使 `enable_thinking = false` 也会生效；对 OpenAI Responses 来说，后者只控制是否请求可见的推理摘要。
- `reasoning_mode = "pro"` 启用 Pro 推理；省略（或设为 `"standard"`）即为标准模式。`reasoning_context` 可设为 `auto`、`current_turn` 或 `all_turns`。这些字段仅适用于官方 OpenAI GPT-5.6 Responses API。
- krew 使用无状态加密推理回放：请求设置 `store = false`，请求返回 `reasoning.encrypted_content`，在本地持久化完整输出项，并在后续轮次原样回放，不依赖服务端 response ID。
- 配置向导会询问推理力度和是否显示摘要；Pro 模式与 reasoning context 仍需手工编辑配置。
- krew 0.12.0 暂不暴露 GPT-5.6 原生 Ultra 模式、Provider 侧多 Agent 编排和 PTC 控制。
...
每个 Provider 需要 `type` 字段指定类型：`"openai"`、`"anthropic"`、`"google"` 或 `"vertex-anthropic"`。
...
# Vertex Anthropic（Google 官方 Vertex endpoint）
[providers.vertex-anthropic]
type = "vertex-anthropic"
api_key_env = "VERTEX_ANTHROPIC_API_KEY" # Bearer token 来源
vertex_project = "my-project"
vertex_location = "global"
...
**`vertex-anthropic` 字段** — `api_key` / `api_key_env` 表示 Bearer token 来源，可以是 Google OAuth access token，也可以是 LiteLLM virtual key。`vertex_project` 和 `vertex_location` 运行时必填。`base_url` 可选；留空表示 Google 官方 Vertex endpoint，填写时可指向 LiteLLM Vertex passthrough root，例如 `https://litellm.example.com/vertex_ai`。
...
1. 选择 Provider 类型（Anthropic / OpenAI / Google / Vertex Anthropic / OpenAI 兼容）
2. 输入 Provider 名称（根据类型自动建议）
3. 选择 API Key 方式（环境变量或配置文件）
4. 可选设置 base_url（OpenAI 兼容或 Vertex Anthropic passthrough）以及 Vertex 字段（Google / Vertex Anthropic）
5. OpenAI 兼容 Provider：选择 API 类型——"Chat Completions（推荐）"（默认）或 "Responses API"。官方 OpenAI Provider 会自动默认使用 Responses API，无需选择。
6. 循环 — 继续添加或完成

> **注意：** 如果 `base_url` 以 `/v1` 结尾（如 `https://openrouter.ai/api/v1`），krew 会自动去掉末尾的 `/v1`，避免 API 请求路径重复。
...
对于官方 GPT-5.6 模型，两种模式都会显示六档推理力度选择器（默认 `medium`）；力度不是 `none` 时，向导接着询问 **Show reasoning summary?**（默认是）。Pro 模式和 reasoning context 需手工配置。
...
| `<消息>`（无 @ 或 #） | 发给上一个回答者；会话刚开始没有上一个回答者时，发给 `reply_order` 中第一个可用的 Agent |
...
- 响应大小限制 1MB
- 通过 `[[allow_rules]]` 中 `tool = "fetch_url"` 配置域名匹配：后缀匹配（如 `docs.github.com` 匹配 `github.com`）
- 超时 30 秒
...
**权限规则**：通过 `[[allow_rules]]`、`[[deny_rules]]`、`[[ask_rules]]` 进行细粒度控制。规则按顺序评估：deny（拒绝）→ ask（强制确认）→ allow（自动放行）。每条规则指定 `tool` 工具名和可选的 `pattern`/`reason`。

**保护路径免疫**：保护路径（`.git/`、`.krew/`、`.vscode/`、`.idea/`、`.claude/`、`.env`、`.bashrc` 等）始终受保护——即使在 `full-auto` 模式下，对这些路径的操作也需要确认，且不可被 allow 规则绕过。**例外**：`.krew/memory/**` 路径自动放行（见 [§14](#14-agent-memory)）。
...
Sub-Agent 允许 Agent 将专项任务委派给在隔离上下文中运行的子代理，避免 tool call 污染主对话。
...
默认关闭。关闭时不会读取任何 agent 定义文件，零开销。

### 定义 Sub-Agent

在以下任一目录放置 `.md` 文件：

- `.krew/agents/` — 项目级（最高优先级）
- `.agents/agents/` — 项目级，跨工具
- `.claude/agents/` — 项目级，兼容 Claude Code

用户级目录（`~/.krew/agents/` 等）也支持。
...
- YAML body（`---` 之后）作为 Sub-Agent 的 system prompt
- Claude Code 的 `tools`、`model` 等字段会被忽略，不影响使用

### 工作原理

1. Agent 通过 `run_agent` tool 调用 Sub-Agent
2. Sub-Agent 在完全隔离的上下文中执行任务（独立消息历史）
3. Sub-Agent 共享父 Agent 的所有工具（含 MCP）和审批配置
4. 执行期间的 tool 调用过程实时流式展示给用户
5. 完成后将最终结果返回给父 Agent

### 查看 Sub-Agent

使用 `/agents` 命令可查看已发现的 Sub-Agent 定义。

---

## 14. Agent Memory

Agent 可以跨会话持久化知识——用户偏好、项目上下文和行为反馈保存在 `.krew/memory/`，每次 Agent turn 时自动加载。

### 工作原理

1. 每次 Agent turn 时，krew 加载 `MEMORY.md` 索引文件并注入到 Agent 的 system prompt 中
2. `tools = true` 的 Agent 收到完整的读写指令，可通过 `write_file` 保存新记忆
3. `tools = false` 的 Agent 仅收到索引内容（只读）

### 两层存储

| 层级 | 路径 | 范围 | 记忆类型 |
| ---- | ---- | ---- | -------- |
| Global | `.krew/memory/` | 所有 Agent 共享 | `user`、`project`、`reference` |
| Per-Agent | `.krew/memory/agents/{name}/` | 单个 Agent 私有 | `feedback` |

- **user** — 用户的角色、目标、偏好、知识
- **project** — 进行中的工作、截止日期、无法从代码推导的决策
- **reference** — 外部资源指针（Issue 追踪器、仪表板、文档）
- **feedback** — 针对特定 Agent 的行为指导（"别用 emoji"、"简洁回答"）
...
.krew/memory/
├── MEMORY.md                    ← 全局索引
├── user_role.md                 ← 全局 topic 文件
├── project_deadline.md
└── agents/
    └── opus/
        ├── MEMORY.md            ← Agent 专属索引
        └── feedback_style.md
```
...
Agent 分两步保存记忆：
1. 将 topic 文件（如 `user_role.md`）写为纯 Markdown
2. 在对应的 `MEMORY.md` 索引中添加一行条目

### 限制

- `MEMORY.md` 在 **200 行**或 **25,000 字节**时截断（取先到者）
- 截断时附加警告信息
- 目录在首次使用时自动创建

### 审批

Memory 路径（`.krew/memory/**`）对 `read_file`、`write_file`、`edit_file` 自动放行——Agent 读写记忆无需用户确认。用户定义的 `deny_rules` 和 `ask_rules` 仍然生效。
...
| `denied` | `agent`, `tool`, `reason` | 工具调用被规则拒绝或非交互模式自动拒绝 |
...
- `-p` 与 `--resume` 互斥
- 工具以 `full-auto` 模式运行，但 **deny 规则**、**ask 规则**和**保护路径免疫**仍然生效。需要用户确认的工具调用（ask 规则或默认审批）会被**自动拒绝**，因为没有交互用户进行审批。可通过 `[[allow_rules]]` 预授权特定工具。
...
| `.krew/memory/` | Agent 记忆文件（全局 + Agent 私有） |
...
| 快捷键 | 操作 |
| ------ | ---- |
| `Enter` | 发送消息（空闲）/ 排队待发送消息（Agent 响应中）/ 换行（队列已满） |
| `Shift+Enter` / `Ctrl+J` | 换行 |
| `↑` / `↓` | 撤销待发送消息（如有）或浏览输入历史 / 浏览输入历史 |
| `@` | 打开 Agent 补全弹窗（含 "all"） |
| `#` | 打开密语目标弹窗（不含 "all"） |
| `/` | 打开 Slash 命令弹窗 |
| `Esc` | 取消当前 Agent 流式输出（之后自动提交待发送消息） |
| `Ctrl+C`（连按两次） | 退出程序 |
...
如果你输入消息时没有 `@` 或 `#` 前缀，krew 会发给上一个回答者。若还没有 Agent 回答过，则发给 `reply_order` 中第一个可用的 Agent。若仍然没有响应，请检查目标 Agent 是否已配置并且 API key 可用。

### 为什么 Shell 一直要求确认？

Shell 命令在 `suggest` 和 `auto-edit` 模式下默认需要确认。要自动放行常用命令，添加 allow 规则：
...
Shell 模式使用 `*` **通配符**：`"cargo *"` 会自动放行 `cargo build`、`cargo test` 等。要跳过所有审批，使用 `--approval-mode full-auto`（请谨慎使用）。
...
检查以下几点：
1. `.md` 文件是否在[发现路径](#18-文件路径与加载优先级)中（如 `.krew/commands/my-command.md`）
